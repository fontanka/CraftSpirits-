...
### БКУ автоперевод (samogon-i-vodka.ru) — гидрозатворный диверт-клапан

**Никаких подвижных частей.** Чистая физика давления:
1. Бутылка приёмника голов герметично закручена крышкой устройства
2. Длинный тонкий **щуп** (70×10mm) идёт от крышки вертикально вниз внутрь
   бутылки до **заданной оператором глубины** (= объём голов до переключения)
3. Капли спирта льются → бутылка дышит через щуп → головы набираются
4. Когда уровень жидкости поднимается до КОНЦА щупа → щуп оказывается под
   водой → атмосферный канал заперт жидкостью
5. Бутылка теперь герметична. Следующие капли создают избыточное давление
6. Давление выталкивает спирт через **боковой выход** на крышке устройства
7. На боковом выходе стоит **контактный датчик** (тот же щуп 70×10mm,
   но снаружи) — он замыкается на ПЕРВОЙ капле тела
8. Сигнал → контроллеру → переключает алгоритм клапана отбора (heads→body
   duty). Физический поток теперь идёт через боковой выход в бутылку тела.

**Объём голов задаётся ТОЛЬКО позицией щупа в бутылке.** Грубо: глубже =
больше голов. Для grain ~150 ml, для фруктовой 250-300 ml.
...
| Триггер тела | Контактный щуп 70×10mm | На боковом выходе устройства автоперевода (НЕ внутри бутылки!) | Первая капля тела замыкает датчик → переключение алгоритма |
...
| 14 | Правильная модель устройства автоперевода: гидрозатвор без подвижных частей; объём голов = позиция щупа; датчик на боковом выходе |
